...
2. Where the history file's last row disagrees with `employees_clean.csv`'s current values (`EMP0356`, `EMP0900` — large salary gaps, source data discrepancies), `employees_clean.csv` wins for the `is_current = TRUE` row — it's documented as the authoritative "current state" export. (`EMP0084` used to be cited here too, but that "disagreement" turned out to be a symptom of a same-day-duplicate SCD2 build bug, not a real data conflict — fixed; see `VALIDATION_EVIDENCE.md`.)
